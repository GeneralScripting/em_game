...
module SimpleEnum
  ...
  module ClassMethods
    ...
    def as_enum(enum_cd, values, options = {})
      ...
      # store info away
      class_attribute :enum_definitions
      if enum_definitions.nil?
      ...
      class_eval(<<-RUBY, __FILE__, __LINE__ + 1)
        ...
        def self.#{attr_name}_for_select(attr = :key, &block)
          if self.constants.include?('#{attr_name.upcase}')
            #{attr_name.upcase}.map do |key|
              [self.human_enum_name(#{attr_name.inspect}, key), key]
            end
          else
